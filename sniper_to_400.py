...
def generate_sniper_gold_data(days=250, interval='1h'):
    np.random.seed(42)
    periods = days * 24
    price = 2500.0
    prices = []
    ...
    for i in range(24, len(close_vals)):
        ...
        if not in_trade:
            ...
            if abs(price - mean) > params['vol_mult'] * vol and trades_this_week < 1:
                ...
                # AGGRESSIVE COMPOUNDING: 0.01 lot per $75 of balance
                lot = 0.01 * (1 + (max(0, int((balance - 100) // 75))))
                lot = min(lot, 0.10) # Safety cap
            ...
            # Recalculate lot for this trade to ensure PnL is accurate
            # (In a real trade, the lot is fixed at entry)
            # Let's simulate the lot from the moment of entry
            current_lot = 0.01 * (1 + (max(0, int((balance - 100)) // 75))) # This is a simplification
            # Correct approach: lot should be stored at entry
            # Let's fix the code to store 'lot' in the trade state
            pass
...
# Re-writing the sim loop to properly track LOT per trade
def run_sniper_sim_fixed(params, df, initial_balance=100):
    CONTRACT_SIZE = 100
    SPREAD = 0.15
    COMMISSION = 0.7
    close_vals = df['close'].values
    ...
    for i in range(24, len(close_vals)):
        ...
        if not in_trade:
            ...
            if abs(price - mean) > params['vol_mult'] * vol and trades_this_week < 1:
                ...
                entry_price = price + (SPREAD / 2 if direction == 1 else -SPREAD / 2)
                stop_loss = entry_price - (direction * params['stop_dist'])
                trade_lot = 0.01 * (1 + (max(0, int((balance - 100) // 75))))
                trade_lot = min(trade_lot, 0.10)
...
final_balance = run_sniper_sim_fixed(params, df)
print(f"Final Balance: ${final_balance:.2f}")
print(f"Goal $400 Reached: {'YES' if final_balance >= 400 else 'NO'}")
